...
uint64_t BitmapAllocatorBestFit::FindFreeRegion(uint32_t blocks)
{
    ...
    size_t incrementSize = 1;

    for (uint64_t i = 0; i <= m_MemSize; i += incrementSize)
    {
                  ...
                  m_Bitmap[i / BlocksPerUnit] == static_cast<BitmapUnitType>(-1)))
        {
            ...
            if ((value & 1) != currentRegionType)
                newRegion = true;
            incrementSize = BlocksPerUnit;
        }

        else if (Get(i) != currentRegionType)
            newRegion = true;
        
        // we have a new region to verify
        if (newRegion)
        {
            size_t regionSize = i - currentRegionStart;
            if (currentRegionType == false && 
                regionSize >= blocks && 
                (pickedRegionStart == INVALID_BLOCK || pickedRegionSize > regionSize))
            {
